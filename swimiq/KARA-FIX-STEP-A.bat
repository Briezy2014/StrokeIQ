@echo off
title SwimIQ - Fix Step A (download stream-v8)
cd /d "%~dp0"
...
echo.
echo ============================================================
echo   Fix STEP A only - download stream-v8 server code
echo ============================================================
echo.
echo Folder: %CD%
echo.
...
if exist "%INDEX%" (
  findstr /C:"2026-gemini-stream-v8" "%INDEX%" >nul 2>&1
  if not errorlevel 1 (
    echo OK - stream-v8 already on this PC.
    goto success
  )
)
...
findstr /C:"2026-gemini-stream-v8" "%INDEX%" >nul 2>&1
if errorlevel 1 (
  echo ERROR - Downloaded file but stream-v8 not found inside.
...
:success
echo.
echo SUCCESS - stream-v8 code is on this PC.
