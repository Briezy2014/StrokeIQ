...
/// Pro-tier highlight reel organizer — tags moments, exports a recruiting
/// highlight package, and on Elite desktop builds a stitched MP4.
...
class _HighlightVideoBuilderScreenState
          ...
          builder: (context, ref, data, swimmer) {
            ...
            return ListView(
              ...
              children: [
                ...
                Text(
                  taggedCount == 0
                      ? 'Tap tags on a video above, then build your recruiting highlight package.'
                      : Env.isPublicHostedWeb
                          ? '$taggedCount video${taggedCount == 1 ? '' : 's'} tagged — '
                              'build a coach-ready highlight package you can copy and share.'
                          : '$taggedCount video${taggedCount == 1 ? '' : 's'} tagged — '
  ...
  Future<void> _generateReel(
    ...
    try {
      if (Env.isPublicHostedWeb) {
        if (!mounted) return;
        await _deliverWebHighlightPackage(
          context: context,
          swimmer: swimmer,
          segments: segments,
        );
      ...
      if (!health.reachable || !health.mediaToolsReady) {
        // Desktop without Elite still gets an active highlight package.
        if (!mounted) return;
        await _deliverWebHighlightPackage(
          context: context,
          swimmer: swimmer,
  ...
  Future<void> _deliverWebHighlightPackage({
    required BuildContext context,
    required String swimmer,
    required List<HighlightReelSegment> segments,
  }) async {
    final buffer = StringBuffer()
      ..writeln('SWIMIQ RECRUITING HIGHLIGHT PACKAGE')
      ..writeln('Athlete: $swimmer')
      ..writeln('Clips: ${segments.length}')
      ..writeln('')
      ..writeln('Suggested coach-ready order:');
    for (var i = 0; i < segments.length; i++) {
      final segment = segments[i];
      buffer.writeln('${i + 1}. ${segment.label} — ${segment.tag}');
    }
    buffer
      ..writeln('')
      ..writeln(
        'Share this package with your coach/family edit list, or open each '
        'tagged video from Video Lab when assembling the final recruiting reel.',
      )
      ..writeln('Generated by SwimIQ · swimiqapp.com');
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Your recruiting clip order is copied to the clipboard. '
                  'Paste it into an email, notes app, or coach message.',
                ...
                FilledButton.icon(
                  ...
                  label: const Text('Copy package again'),
                ),
                const SizedBox(height: 8),
                OutlinedButton(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Highlight package copied — ready to share.'),
      ),
    );
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  Env.isPublicHostedWeb
                      ? 'Tap Build highlight package to copy this coach-ready order.'
